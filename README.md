...
- `NovaLauncher-Setup-1.2.2-unsigned-win-x64.exe` for the Windows installer.
- `NovaLauncher-1.2.2-unsigned-win-x64-portable.zip` if you prefer a portable copy.
...
```powershell
Get-FileHash .\NovaLauncher-Setup-1.2.2-unsigned-win-x64.exe -Algorithm SHA256
```
...
`v1.2.2-unsigned` includes NovaLauncher's professional UI overhaul and corrects multi-peer request authorization. Every active trusted device can now access authenticated save and game-transfer operations instead of being rejected by the legacy single-peer pin. Paused, revoked, unknown, and cryptographically unauthenticated devices remain blocked, while clock-skew and trust failures now have separate guidance. Because this build has no publisher certificate, update installation and signed rollback fail closed and remain unavailable. Physical multi-device, controller, display-scale, antivirus, interruption, and very-large-transfer qualification remain open.
