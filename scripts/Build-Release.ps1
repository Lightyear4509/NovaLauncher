...
if (-not (Test-Path $SigningCertificatePath -PathType Leaf)) { throw 'The signing certificate is missing.' }
...
if ([string]::IsNullOrWhiteSpace($signTool)) { throw 'A Windows SDK x64 signtool.exe is required for signed packaging.' }
...
$portable = Join-Path $release 'NovaLauncher-1.2.0-win-x64-portable.zip'
...
& $signTool sign /fd SHA256 /td SHA256 /tr 'http://timestamp.digicert.com' /f $SigningCertificatePath /p $SigningCertificatePassword (Join-Path $release 'NovaLauncher-Setup-1.2.0-win-x64.exe')
if ($LASTEXITCODE -ne 0) { throw 'Installer Authenticode signing failed.' }
$installerSignature = Get-AuthenticodeSignature (Join-Path $release 'NovaLauncher-Setup-1.2.0-win-x64.exe')
if ($installerSignature.Status -ne 'Valid') { throw "Signed installer verification failed: $($installerSignature.Status)." }

& (Join-Path $PSScriptRoot 'New-Sbom.ps1') -OutputPath (Join-Path $release 'NovaLauncher-1.2.0.cdx.json')
