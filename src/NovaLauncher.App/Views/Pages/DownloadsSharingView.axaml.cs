...
public sealed partial class DownloadsSharingView : NovaPage
{
    public DownloadsSharingView() => AvaloniaXamlLoader.Load(this);
    ...
    private async void OnRefreshGameTransferHistory(object? sender, RoutedEventArgs e) => await ExecuteAsync(() => Workspace.RefreshGameTransferHistoryAsync(LifetimeToken));
    private async void OnRefreshSharing(object? sender, RoutedEventArgs e) => await ExecuteAsync(async () =>
    {
        await Workspace.RefreshPeerGameTransferOffersAsync(LifetimeToken);
        await Workspace.RefreshGameTransferHistoryAsync(LifetimeToken);
    });
